...
#' not be considered.
#' @param cowt A logical value (default = TRUE). Compute the Olden's connection
#' weights of the input node (variables), expressed relative to the maximum.
#' @param thr A numerical value indicating the threshold to apply on the absolute
#' values of the connection matrix to color the graph. If thr=NULL (default), the
#' threshold is set to thr = abs(mean(cowt)).
...
#' @param lambda A numerical value indicating the strength of the regularization for 
#' lambda penalty, \eqn{\lambda (L1 + L2)} (default = 0). 
#' @param alpha A numerical vector to add L1/L2 regularization into the training. 
#' Set the alpha parameter for each layer to \eqn{(1 - \alpha) L1 + \alpha L2}. 
...
#' and the data.frame of estimated connection weights (parameterEstimates), if cowt=TRUE.
#' \item "gest", the data.frame of estimated connection weights (parameterEstimates)
#' of outcome levels, if cowt = TRUE and outcome != NULL.
...
#' summed over the outcome levels, i.e. sum(abs(W[outcome levels])) > thr, will be
#' highlighted in pink.
...
#'             lm model.matrix predict p.adjust quantile reshape rnorm runif sd
...
#'
#' start<- Sys.time()
#' dnn1<- SEMdnn(ig1, data[train, ], cowt = TRUE, thr = NULL,
#' 			hidden = 5*K, link = "selu", bias = TRUE,
#'			validation = 0, epochs = 32, ncores = 2)
#' end<- Sys.time()
#' print(end-start)
...
#'
#' start<- Sys.time()
#' dnn2<- SEMdnn(ig, data[train, ], outcome[train], cowt = TRUE, thr = NULL,
#' 			#hidden = 5*K, link = "selu", bias = TRUE,
#' 			hidden = c(10,10,10), link = "selu", bias = TRUE,
...
#' @export
#'
SEMdnn <- function(graph, data, outcome = NULL, cowt = TRUE, thr = NULL,
                   ...
                   early_stopping = FALSE, verbose = FALSE, ...)
{
  ...
  res <- parameterEstimates.DNN(dag, data, hidden, link, bias, dropout, loss,
  ...
  if (cowt) {
    if (!is.null(outcome)) {
      ...
      dnn <- list(model=res[[1]], gest=gest, graph=dag-levels(outcome))
      cowt <- getConnectionWeight(object=dnn, thr=thr, verbose=FALSE)
      gest <- cowt$est[cowt$est$lhs %in% levels(outcome), ]
    }else{
      dnn <- list(model=res[[1]], gest=gest, graph=dag)
      cowt <- getConnectionWeight(object=dnn, thr=thr, verbose=FALSE)
    }
    est <- cowt$est
    dag <- cowt$dag
...
parameterEstimates.DNN <- function(dag, data, hidden, link, bias, dropout, loss,
                                   validation, lambda, alpha, optimizer, lr, epochs,
                                   device, ncores, early_stopping, verbose, ...)
{
  # Set objects:
...
#' observation.
#
#' @return A list od two object: (i) est: a data.frame including the connections together
...
{
  stopifnot(inherits(object$model[[1]], "citodnn"))
  model <- object$model
  dag <- object$graph
  ...
  return(list(est = dag0$est, dag = dag0$dag))
}

colorDAG <- function(dag, est, out, thrV=NULL, thrE=NULL, verbose=FALSE, ...)
{
  # set node colors
  if (!is.null(out)) {
    dag <- dag - out
    estC <- est[est$lhs %in% out, ]
    sumC <- aggregate(abs(estC[ ,4])~estC$rhs,FUN="sum")
    colnames(sumC) <- c("node", "varImp")
    sumC$varImp <- sumC$varImp/max(sumC$varImp, na.rm=TRUE)
    V(dag)$weight <- sumC$varImp[match(V(dag)$name, sumC$node)]
    if (is.null(thrV)) thrV <- mean(abs(V(dag)$weight), na.rm=TRUE)
    V(dag)$color <- ifelse(V(dag)$weight > thrV, "pink", "white")
  }
  
  # set edge colors
  if (!is.null(out)) est <- est[!est$lhs %in% out, ]
  est[ ,4] <- est[ ,4]/max(abs(est[ ,4]), na.rm=TRUE)
  E0 <- paste0(est$rhs,"|",est$lhs)
  E1 <- attr(E(dag), "vnames")
  E(dag)$weight <- est[ ,4][match(E1, E0)]
  if (is.null(thrE)) thrE <- mean(abs(E(dag)$weight), na.rm=TRUE)
  
  enames <- attr(E(dag), "vnames")
  Er <- enames[abs(E(dag)$weight) > thrE & E(dag)$weight < 0]
  Ea <- enames[abs(E(dag)$weight) > thrE & E(dag)$weight > 0]
  ...
  return(list(est = est[match(E1, E0), ], dag = dag))
}
...
#'
#' @return A list od two object: (i) est: a data.frame including the connections together
#' with their weights, and (ii)dag:  the DAG with colored edges. If abs(W) > thr and W < 0,
#' the edge is inhibited and it is highlighted in blue; otherwise, if abs(W) > thr and
#' W > 0, the edge is activated and it is highlighted in red. If the outcome vector is
#' given, nodes with absolute connection weights summed over the outcome levels, i.e.
#' sum(abs(W[outcome levels])) > thr, will be highlighted in pink.
#'
#' @author Mario Grassi \email{mario.grassi@unipv.it}
#'
#' @references
...
getConnectionWeight <- function(object, thr = NULL, verbose = FALSE, ...)
{
  ...
  model <- object$model
  stopifnot(inherits(model[[1]], "citodnn"))
  nrep <- ifelse(inherits(object, "ML"), length(model)-1, length(model))
  dag <- object$graph
  out <- unique(object$gest$lhs)
  if (!is.null(out)) {
    dag <- mapGraph(dag, type="outcome", C=length(out))
...
#'
#' @param object A neural network object from \code{SEMdnn()} function. 
#' @param thr A value of the threshold to apply to input t_test-values. If
...
#' 
#' @return A list od two object: (i) est: a data.frame including the connections
...
getSignificanceTest<- function(object, thr = NULL, verbose = FALSE, ...)
{
  ...
  rownames(est) <- NULL
  class(est) <- c("lavaan.data.frame","data.frame")
  #p.adj <- p.adjust(est$pvalue, method="BH")
  #weight <- sign(est[,4]) * (1 - p.adj)
  #est <- cbind(est[,c(1:3)], weight)
  #thr <- ifelse(is.null(thr), 0.95, 1-thr)
  est <- est[ ,c(1:3,6)]
